...
static QTreeWidgetItem *XServer_fill_screen_info(QTreeWidgetItem *lBox, QTreeWidgetItem *last, Display *dpy, int scr, int default_scr)
{
    ...
    dimensionList << i18n("Dimensions")
                  << i18n("%1 x %2 Pixels (%3 x %4 mm)",
    ...
    optionList << i18n("Options")
               << i18n("backing-store: %1, save-unders: %2",
                       (DoesBackingStore(s) == NotUseful) ? NO : ((DoesBackingStore(s) == Always) ? YES : i18n("When mapped")),
...
static bool GetInfo_XServer_Generic(QTreeWidget *lBox)
{
    ...
    if (vendor.contains(QStringLiteral("x.org"), Qt::CaseInsensitive)) {
        ...
        releaseString = QVersionNumber(releaseNumber / 10000000, (releaseNumber / 100000) % 100, (releaseNumber / 1000) % 100).toString();
    }
    ...
    if (pmf) {
        for (i = 0; i < n; i++) {
            ...
            pixmapList << i18n("Pixmap Format #%1", i + 1)
                       << i18n("%1 BPP, Depth: %2, Scanline padding: %3", pmf[i].bits_per_pixel, BitString(pmf[i].depth), BitString(pmf[i].scanline_pad));
