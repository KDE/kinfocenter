...
static QTreeWidgetItem* addDeviceClass(QTreeWidgetItem *parent, QTreeWidgetItem *after, pciInfo *info) {
	QTreeWidgetItem *localAfter=nullptr;
        ...
        after=create(parent, i18n("Device Subclass"), getNameBy2Id(devSubclass, info->cooked.devClass, info->cooked.devSubClass)+QString::asprintf(" (0x%02X)", info->cooked.devSubClass));
        after=create(parent, i18n("Device Programming Interface"), getNameBy3Id(devInterface, info->cooked.devClass, info->cooked.devSubClass, info->cooked.devProgIface.devProgIface)+QString::asprintf(" (0x%02X)", info->cooked.devProgIface.devProgIface));
...
static QTreeWidgetItem* addVendor(QTreeWidgetItem *parent, QTreeWidgetItem *after, pciInfo *info, pci_access *PCIAccess) {
	...
	if ((!isSub)&&(info->cooked.headerType.headerType_bits.headerType!=PCI_HEADER_TYPE_BRIDGE)) { //if entire subsytem was not found, search at least for subvendor
		if (pci_lookup_name(PCIAccess, nameBuffer, NAME_BUFFER_SIZE, PCI_LOOKUP_VENDOR, subvendor, 0, 0, 0)!=nullptr) {
			...
			if (!line.contains(QLatin1String("Unknown"))) {
                                after=create(parent, i18n("Subsystem"), line+i18n(" - device:")+QString::asprintf(" 0x%04X (0x%04X:0x%04X)", subdevice, subvendor, subdevice));
			}//if
			else {
	...
	if (info->cooked.headerType.headerType_bits.headerType==PCI_HEADER_TYPE_NORMAL) {
		if (info->cooked.header.header0.minGnt==0) {
			localAfter=create(after, i18n("MIN_GNT"), i18n("No major requirements (0x00)"));
                        ...
                        localAfter=create(after, i18n("MIN_GNT"), QString::asprintf("%u ns (0x%02X)", info->cooked.header.header0.minGnt*250, info->cooked.header.header0.minGnt));
		}//else
		if (info->cooked.header.header0.maxLat==0) {
			localAfter=create(after, i18n("MAX_LAT"), i18n("No major requirements (0x00)"));
		}//if
		else {
                        localAfter=create(after, i18n("MAX_LAT"), QString::asprintf("%u ns (0x%02X)", info->cooked.header.header0.maxLat*250, info->cooked.header.header0.maxLat));
		}//else
	}//if
	return after;
}//addLatency
...
static QTreeWidgetItem* addHeaderType(QTreeWidgetItem *parent, QTreeWidgetItem *after, pciInfo *info) {
	QTreeWidgetItem *localAfter=nullptr;
        ...
        localAfter=create(after, i18n("Type"),getNameById(headerType,info->cooked.headerType.headerType_bits.headerType)+QString::asprintf(" (0x%02X)",info->cooked.headerType.headerType_bits.headerType));
...
static QTreeWidgetItem* addMapping(QTreeWidgetItem *parent, QTreeWidgetItem *after, pciInfo *info, pci_dev* PCIDevice) {
	...
	for (int i=0; i<6; i++) {
		...
		if (is64b) { //there is no long long support in Qt so we need compose value
                        topLocalAfter->setText(1, QString::asprintf("0x%08X%08X", info->cooked.header.header0.mapping[i+1].baseAddress, info->cooked.header.header0.mapping[i].baseAddress));
			if (info->cooked.header.header0.mapping[i+1].baseAddress==0) { //no top 4 bytes
				if ((info->cooked.header.header0.mapping[i].baseAddress&(~(info->cooked.header.header0.mapping[i].baseAddress_bits.baseAddressMap ? 0x3 : 0xF)))==0) { //no address at all
					...
					localAfter=create(topLocalAfter, i18n("Size"), i18nc("unassigned size", "Unassigned"));
                                        ...
                                        localAfter=create(topLocalAfter, i18n("Address"), QString::asprintf("0x%X", (info->cooked.header.header0.mapping[i].baseAddress&(~(info->cooked.header.header0.mapping[i].baseAddress_bits.baseAddressMap ? 0x3 : 0xF)))));
					localAfter=addSize(topLocalAfter, localAfter, PCIDevice->size[i]);
				}//else
			}//if
			else {
                                localAfter=create(topLocalAfter, i18n("Address"), QString::asprintf("0x%X%08X", info->cooked.header.header0.mapping[i+1].baseAddress, (~(info->cooked.header.header0.mapping[i].baseAddress&(info->cooked.header.header0.mapping[i].baseAddress_bits.baseAddressMap ? 0x3 : 0xF)))));
				localAfter=addSize(topLocalAfter, localAfter, PCIDevice->size[i]);
			}//else
		}//if
		else {
...
static QTreeWidgetItem* addBridgeBehind(QTreeWidgetItem *parent, QTreeWidgetItem *after, pciInfo *info) {
	...
	if (info->cooked.headerType.headerType_bits.headerType==PCI_HEADER_TYPE_BRIDGE) {
		...
		if (info->cooked.header.header1.ioBase.ioBase_bits.ioBaseType==0) {
                        ...
                        localAfter=create(after, i18n("Base"),QString::asprintf("0x%04X%04X",info->cooked.header.header1.ioBaseUpper16,info->cooked.header.header1.ioBase.ioBase&0xFFF0));
                        localAfter=create(after, i18n("Limit"),QString::asprintf("0x%04X%04X",info->cooked.header.header1.ioLimitUpper16,info->cooked.header.header1.ioLimit|0x0F));
		}//else
		...
		if (info->cooked.header.header1.ioBase.ioBase_bits.ioBaseType==0) {
                        localAfter=create(after, i18n("Base"),QString::asprintf("0x%08X",(info->cooked.header.header1.prefMemoryBase.prefMemoryBase<<16)&0xFFFFFFF0));
                        localAfter=create(after, i18n("Limit"),QString::asprintf("0x%08X",(info->cooked.header.header1.prefMemoryLimit<<16)|0x0FFFFF));
		}//if
		else {
                        localAfter=create(after, i18n("Base"),QString::asprintf("0x%08X%08X",info->cooked.header.header1.prefBaseUpper32,(info->cooked.header.header1.prefMemoryBase.prefMemoryBase<<16)&0xFFFFFFF0));
                        localAfter=create(after, i18n("Limit"),QString::asprintf("0x%0x8X%08X",info->cooked.header.header1.prefLimitUpper32,(info->cooked.header.header1.prefMemoryLimit<<16)|0x0FFFFF));
		}//else
	}//if
	return after;
}//addBridgeBechind
	...
	if (info->cooked.headerType.headerType_bits.headerType==PCI_HEADER_TYPE_CARDBUS) {
		...
		for (int i=0; i<2; i++) {
			...
			if (info->cooked.header.header2.cbIo[i].cbIoBase.cbIoBase_bits.cbIoBaseType==1) {
                                localAfter=create(topLocalAfter, i18n("Base"),QString::asprintf("0x%08X",info->cooked.header.header2.cbIo[i].cbIoBase.cbIoBase&0xFFFFFFFC));
		...
		if (infoAgp.cooked.status.status_bits1.statusMode==1) {
			...
			for (i=0, cycleSize=1; i<(infoAgp.cooked.status.status_bits1.statusOptReqSize+4); i++) {
				cycleSize*=2;
                        ...
                        localAfter=create(after, i18n("Optimum asynchronous request size"),QString::asprintf("%i (0x%02X)",cycleSize,infoAgp.cooked.status.status_bits1.statusOptReqSize));
			localAfter=create(after, i18n("Isochronous transactions"),(infoAgp.cooked.status.status_bits1.statusIsochSupp?i18nc(strCtxt, strEnabled):i18nc(strCtxt, strDisabled)));
                ...
                localAfter=create(after, i18n("Maximum number of AGP command"),QString::asprintf("%i (0x%02X)",infoAgp.cooked.status.status_bits1.statusReq+1,infoAgp.cooked.status.status_bits1.statusReq));
		localAfter=nullptr;
		...
		if (infoAgp.cooked.status.status_bits1.statusMode==1) {
			...
			for (i=0, cycleSize=1; i<(infoAgp.cooked.config.config_bits1.configOptReqSize+4); i++) {
				cycleSize*=2;
                        ...
                        localAfter=create(after, i18n("Optimum asynchronous request size"),QString::asprintf("%i (0x%02X)",cycleSize,infoAgp.cooked.config.config_bits1.configOptReqSize));
		}//if
                localAfter=create(after, i18n("Maximum number of AGP command"),QString::asprintf("%i (0x%02X)",infoAgp.cooked.config.config_bits1.configReq+1,infoAgp.cooked.config.config_bits1.configReq));
	}//if
	return after;
}//addCapsAgp
	...
	if ((offset+4)<256) { //copy control only (for now)
		...
		if ((offset+size)<256) { //copy all MSI data
			...
			if (infoMsi.cooked.msiControl.msiControl_bits.msi64bit==1) {
                                after=create(parent, i18n("Address"),QString::asprintf("0x%08X%08X",infoMsi.cooked.payload.address.msiUpperAddress,infoMsi.cooked.msiAddress));
		...
		if ((info->cooked.header.header0.capabilityList!=0)&&(info->cooked.status.status_bits.statCapList!=0)) {
			...
			if (getuid()==0) {
				for (offset=info->cooked.header.header0.capabilityList; offset!=0; offset=info->raw[offset+1]) {
                                        ...
                                        localAfter=create(topLocalAfter, i18n("Next"),(info->raw[offset+1]==0?i18n("0x00 (None)"):QString::asprintf("0x%02X",info->raw[offset+1])));
