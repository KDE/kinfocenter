 ...
 *  SPDX-FileCopyrightText: 2004 Ilya Korniyko <k_ilya@ukr.net>
 *  Adapted from Brian Paul's glxinfo from Mesa demos (https://www.mesa3d.org/)
 *  SPDX-FileCopyrightText: 1999-2002 Brian Paul
 *
...
K_PLUGIN_FACTORY(KCMOpenGLFactory, registerPlugin<KCMOpenGL>();)
    ...
    : KCModule(parent)
{
    ...
    KAboutData *about = new KAboutData(i18n("kcmopengl"),
...
static QVector<DriInfo> get_dri_devices()
{
    ...
    for (auto it = infos.begin(); it != infos.end(); ++it) {
        ...
        int num = 0;
        if (((num = ReadPipe(cmd, pci_info)) || (num = ReadPipe("/sbin/" + cmd, pci_info)) || (num = ReadPipe("/usr/sbin/" + cmd, pci_info))
             || (num = ReadPipe("/usr/local/sbin/" + cmd, pci_info)))
            && num >= 7) {
            QString line;
            for (int i = 2; i <= 6; i++) {
                line = pci_info[i];
...
static void mesa_hack(Display *dpy, int scrnum)
{
    static const int attribs[] = {
        GLX_RGBA, GLX_RED_SIZE,         1, GLX_GREEN_SIZE,      1, GLX_BLUE_SIZE,        1, GLX_DEPTH_SIZE,   1,    GLX_STENCIL_SIZE, 1, GLX_ACCUM_RED_SIZE,
        1,        GLX_ACCUM_GREEN_SIZE, 1, GLX_ACCUM_BLUE_SIZE, 1, GLX_ACCUM_ALPHA_SIZE, 1, GLX_DOUBLEBUFFER, None,
    };
    XVisualInfo *visinfo;
...
static void print_limits(QTreeWidgetItem *l1, const char *glExtensions, bool getProcAddress)
{
    /*  TODO
         GL_SAMPLE_BUFFERS
         GL_SAMPLES
...
#define KCMGL_SIZE(x) (sizeof(x) / sizeof(x[0]))
    ...
    for (uint i = 0; i < KCMGL_SIZE(groups); i++) {
        if (groups[i].ext && !strstr(glExtensions, groups[i].ext))
            continue;
        ...
        for (cur_token = groups[i].group; cur_token->type; cur_token++) {
            ...
            if (glGetError() == GL_NONE) {
                ...
                if (!tfloat && count == 1)
                    ...
                    s = QStringLiteral("%1, %2").arg(max[0]).arg(max[1]);
                else if (tfloat && count == 2)
                    s = QStringLiteral("%1 - %2").arg(fmax[0], 0, 'f', 6).arg(fmax[1], 0, 'f', 6);
...
static QTreeWidgetItem *get_gl_info_glx(Display *dpy, int scrnum, Bool allowDirect, QTreeWidgetItem *l1, QTreeWidgetItem *after)
{
    ...
    const int attribSingle[] = {GLX_RGBA, GLX_RED_SIZE, 1, GLX_GREEN_SIZE, 1, GLX_BLUE_SIZE, 1, None};
